...
def conjunct(ineq_dict1, ineq_dict2):
    ...
    for key in shared_keys:
        ...
        new_ineq = Inequality(ineq1.var, min(ineq1.lwrbnd, ineq2.lwrbnd), max(
...
class Transition:
    def __init__(self, ineq=True, inp=None, out=None, inter=None, s1=None, s2=None):
        # guard is a set of inequalities joined by conjunction (?)
        self.guard = ineq
...
def compose_transitions(tr1, tr2):
    ...
    inter = (tr1.input.intersection(tr2.output)).union(
        tr1.output.intersection(tr1.input))
...
class ComponentAutomaton:
    ...
    def add_state(self, name, transitions, end_state=0):
        ...
        self.transitions_dict[name.upper()] = map(
            lambda x: x.upper(), transitions)
    ...
    def simulate(self, is_printing=False):
        ...
        while state not in self.endStates:
            ...
            # sample from set of all transistions uniformly at random
            state, = sample(self.transitions_dict[state], 1)
    ...
    def compose_components(component_1, component_2):
        ...
        newtransitions = []
        newends = itertools.product(
            component_1.endStates, component_2.endStates)
        dict1 = component_1.transitions_dict
        dict2 = component_2.transitions_dict
        for key1 in dict1:
            for key2 in dict2:
                newtransitions.append(
                    compose_transitions(dict1[key1], dict2[key2]))
