...
def vertices_pedestrian(x, y):
    ...
    # counter clockwise
    return [(x - w1, y), (x, y - h2), (x + w2, y), (x, y + h1)]
...
def overlap(s1, s2, axis):
    if (s1[1] < s2[0] or s2[1] < s1[0]):
        ...
        # (max2 - min1, max1 - min2) gets the distance of of penetration
        d = min(s2[1] - s1[0], s1[1] - s2[0])
        # removes the scalar of axis from earlier projection between vertex and axis
        factor = d / dot(axis, axis)
...
def get_bounding_box(thing):
    if type(thing) is Pedestrian:
        ...
        # the longest distance used for quick circular bounding box
        radius = 40 * params.pedestrian_scale_factor
        ...
        radius = ((788 * params.car_scale_factor / 2) ** 2 +
                  (399 * params.car_scale_factor / 2) ** 2) ** 0.5
    else:
        TypeError('Not sure what this object is')

    rotated_vertices = [rotate_vertex(x, y, theta, vertex)
    ...
    ordered_vertices = rotated_vertices[min_index:] + \
        rotated_vertices[:min_index]
...
def nonoverlapping_polygons(polygon1_vertices, polygon2_vertices):  # SAT algorithm
    ...
    edges = vectors_of_edges(polygon1_vertices) + \
        vectors_of_edges(polygon2_vertices)
    ...
    # if min_sep_vector pointing in same direction as this vector, used to invert direction min_sep_vector
    vector_of_centers = (center1[0] - center2[0], center1[1] - center2[1])

    # assume this is True initially, we will check if this is actually the case
    all_overlapping = True
    # look for overlapping in projections to each axis
    for i in range(len(axes)):
        # dots all of the vertices with one of the separating axis and returns the (min, max) projections
        projection_a = projection(polygon1_vertices, axes[i])
        projection_b = projection(polygon2_vertices, axes[i])
        # (min,max) of both polygons are compared for overlap
        overlapping, sv = overlap(projection_a, projection_b, axes[i])
        # adds the vector needed to separate the polygons along the axis
        separation_vectors.append(sv)
        # check if all_overlapping is still True
        all_overlapping = all_overlapping and overlapping
    ...
    # gets the smallest vector needed to separate the objects
    min_sep_vector = min(separation_vectors, key=(lambda v: dot(v, v)))
    # if vectors are the same direction, invert min_sep_vector, this is for consistency in finding contact points later
    if dot(min_sep_vector, vector_of_centers) > 0:
...
# the closest edge is the edge most perpendicular to the separation normal
def best_edge(polygon, separation_normal):
...
def clip_points(v1, v2, n, o):  # clips line segment points v1, v2 if they are past o along n
    ...
    if d1 * d2 < 0.0:  # checks if the points are on opposing sides to compute the correct point
        # v2 - v1, gets the vector of edge thats being clipped
...
def contact_points(object1, object2, separation_normal):
    # keep consistent separation normal from object 2 to object 1
    invert_normal = invert_direction(separation_normal)
    ...
    # offset of first vertex along ref edge vector
    offset1 = dot(ref_edge, ref_v1)
    # clips the incident edge by the first vertex of the reference edge
    cp = clip_points(inc_v1, inc_v2, ref_edge, offset1)
    if len(cp) < 2:  # less than two points then it fails and returns
        return cp

    # offset, second vertex along ref edge vector
    offset2 = dot(ref_edge, ref_v2)
    # clips what's left of incident edge by the second vertex of the reference edge in the opposite direction, the offset and direction are flipped
    cp = clip_points(cp[0], cp[1], ref_v_invert, -offset2)
    if len(cp) < 2:
        return cp

    # reference edge normal towards its own center
    ref_normal = invert_direction(get_axis(ref_edge))
...
def get_impulse(object1, object2, cp, min_sep_vector):  # need to test
    ...
    # vector from center towards contact point
    r_a = (cp[0][0] - x, cp[0][1] - y)
    # vector from center towards contact point
    r_b = (cp[0][0] - x2, cp[0][1] - y2)
    ...
    # returns best edge of object2
    edge_obj2, _, _, _ = best_edge(object2, min_sep_vector)
    ...
    j = -(1 + e) * dot(v_AB, n)
    j /= dot(n, n) * ((1 / m_a) + (1 / m_b)) + (dot(r_a, n)
                                                ** 2 / inert_a) + (dot(r_b, n) ** 2 / inert_b)
    return j
